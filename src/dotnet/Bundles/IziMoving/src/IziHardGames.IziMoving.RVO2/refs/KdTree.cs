...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            // для каждой ноды устанавливается начальное значение = позиции начальной ноды
            ...
            // от следующего агента до последнего агента
            for (int i = begin + 1; i < end; ++i)
            {
                // выбираются крайние значение позиции среди всех агентов. Самая дальняя и самая ближняя позиция относительно текущей ноды агнета
